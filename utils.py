...
def write_cls_summary(image_resolution, model, model_input, gt,
                        model_output, writer, total_steps, prefix='train_', target='sobel'):
    pass

def write_image_summary(image_resolution, model, model_input, gt,
                        model_output, writer, total_steps, prefix='train_', target='sobel'):
    gt_img = dataio.lin2img(gt['img'], image_resolution)# * 256
    pred_img = dataio.lin2img(model_output['model_out'], image_resolution)
    out_img = pred_img# * 256

    img_gradient = diff_operators.gradient(model_output['model_out'], model_output['model_in'])
    if 'new_img' in model_output:
        sobel = dataio.lin2img(model_output['new_img'], image_resolution)
    else:
        sobel_x = img_gradient[..., :1]
        sobel_y = img_gradient[..., 1:]
        sobel_x = dataio.lin2img(sobel_x, image_resolution)
        sobel_y = dataio.lin2img(sobel_y, image_resolution)
    ...
    writer.add_image(prefix + 'gt_vs_pred', make_grid(output_vs_gt, scale_each=False, normalize=True),
                     global_step=total_steps)
    pred_img = dataio.rescale_img((out_img), mode='clamp').permute(0,2,3,1).squeeze(0).detach().cpu().numpy()
    sobel_img = dataio.rescale_img((sobel), mode='clamp').unsqueeze(-1).detach().cpu().numpy()
    ...
    gt_img = dataio.rescale_img(gt_img, mode='clamp').permute(0, 2, 3, 1).squeeze(0).detach().cpu().numpy()
    print(gt['gradients'].shape)
    gt_grad = dataio.rescale_img(dataio.lin2img(gt['gradients'])).permute(0, 2, 3, 1).squeeze(0).detach().cpu().numpy()

    writer.add_image(prefix + 'pred_img', torch.from_numpy(pred_img).permute(2, 0, 1), global_step=total_steps)
    writer.add_image(prefix + 'sobel_img', torch.from_numpy(sobel_img).permute(2, 0, 1), global_step=total_steps)
